...
def Run(categorical=True, Normalize=False, shuffle=True):
    ...
    model.summary()
    model.count_params()
    iterations = (X_train.shape[0]*6)
    valid_iteration = (X_valid.shape[0]*4)
    ...
    #
    clr = cyclical_learning_rate.CyclicLR(base_lr=1e-4, max_lr=0.006,
                                step_size=iterations//2, mode='exp_range')
    checkpoint = callbacks.ModelCheckpoint('./105_adam_cycle_weights-{epoch:02d}.h5', monitor='val_dice_coef', #monitor='val_ucent_jaccard_distance',

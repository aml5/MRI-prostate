...
def AutoEncoder(x,init):
    x_a = layers.Conv3D(filters=8, kernel_size=(4,4,4), strides = (1,1,1), kernel_initializer=init, padding='same')(x)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    x_a = layers.MaxPooling3D(pool_size=(2,2,2), padding='same')(x_a)
    #36x4
    x_a = layers.Conv3D(filters=4, kernel_size=(4,4,4), strides=(1,1, 1), padding='same',kernel_initializer=init)(x_a)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    x_a = layers.MaxPooling3D(pool_size=(2,2,2), padding='same')(x_a)
    #18x4
    x_a = layers.Conv3D(filters=1, kernel_size=(4,4,4), strides=(1,1, 1), padding='same',kernel_initializer=init)(x_a)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    ...
    x_a = layers.Conv3D(filters=6, kernel_size=(4,4,4), strides=(1,1, 1), padding='same',kernel_initializer=init)(x_a)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    #144x16
    x_a = layers.UpSampling3D(size=(2,2,2))(x_a)
    x_a = layers.Conv3D(filters=8, kernel_size=(4,4,4), strides=(1,1, 1), padding='same',kernel_initializer=init)(x_a)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    x_a = layers.Conv3D(filters=1, kernel_size=(4,4,4), strides=(1,1, 1), padding='same',kernel_initializer=init)(x_a)
    return layers.Activation('tanh',  name='autoencoder')(x_a), x_a_Con


def D3GenerateModel(n_filter=16, number_of_class=1, input_shape=(16,144,144,1),activation_last='sigmoid', metrics=['mse', 'binary_accuracy', dice_coef], loss=['mse','mse'], dropout=0.05, init='glorot_uniform', two_output=False):
    ...
    x = layers.concatenate([dist_img, input_x_centered, stroma, input_x_t])
    #16x144x144x1--> 8x72x72x16
    filter_size = n_filter
    x = layers.Conv3D(filters=filter_size, kernel_size=(2,5,7), strides = (1,1,1), kernel_initializer=init, padding='same')(x)
    x = cyclical_learning_rate.SineReLU()(x)
    x = layers.Conv3D(filters=filter_size, kernel_size=(2,5,7), strides=(1,1, 1), 
    ...
    conv_list = []
    counter = 0
    x = layers.Conv3D(filters=filter_size*2, kernel_size=(2,5,7), strides=(1,1, 1), 
    ...
    x = layers.Conv1D(filters=number_of_class, kernel_size=reduce_filter, strides=reduce_filter, kernel_initializer=init, kernel_regularizer=regularizers.l2(0.001))(x)
    x = layers.Activation('sigmoid')(x)
    y = layers.Flatten(name='prediction')(x)
    #Classification
    model = Model(inputs=input_x, outputs=[loss_opt,y])
    import yogi #yogi.Yogi(lr=lr) # 
    optimizer = optimizers.adam(lr=lr, amsgrad=True)#yogi.Yogi(lr=lr) 
...
def GenerateEqualPositiveAndNegativeValue(X,Y, batch_size=16, RunNormalize=True, max_iteration_per_epoch=10000, RunApplyAugmentation=False):
    ...
    while 1:
        ...
        yield x_batch_tmp, [x_batch_tmp_c,y_batch_tmp]

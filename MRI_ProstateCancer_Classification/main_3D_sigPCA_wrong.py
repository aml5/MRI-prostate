...
def softmax(x, axis=-1):
    ...
    ex = ex / K.sum(ex, axis=axis, keepdims=True)
    ex_x = tf.Print(K.min(ex), [K.shape(ex)])
    ex_v = tf.Print(K.max(ex), [K.shape(ex)])
    ex_n = tf.Print(K.mean(ex), [K.shape(ex)])
    return ex
...
def AutoEncoder(x,init):
    x_a = layers.Conv3D(filters=6, kernel_size=(5,16,16), strides = (1,1,1), kernel_initializer=init, padding='same', kernel_constraint=min_max_norm(min_value=-1.0, max_value=1.0))(x) #
    x_a = layers.LeakyReLU()(x_a) #cyclical_learning_rate.SineReLU()(x_a)
    x_a = layers.MaxPooling3D(pool_size=(2,2,2), padding='same')(x_a)
    #36x4
    x_a = layers.Conv3D(filters=3, kernel_size=(5,16,16), strides=(1,1, 1), padding='same',kernel_initializer=init, kernel_constraint=min_max_norm(min_value=-1.0, max_value=1.0))(x_a) #, kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
    x_a = layers.LeakyReLU()(x_a)
    x_a = layers.MaxPooling3D(pool_size=(2,2,2), padding='same')(x_a)
    #18x4
    x_a = layers.Conv3D(filters=1, kernel_size=(5,16,16), strides=(1,1, 1), padding='same',kernel_initializer=init, kernel_constraint=min_max_norm(min_value=-1.0, max_value=1.0))(x_a) #, kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
    x_a = layers.LeakyReLU()(x_a)
    x_a_Con = layers.MaxPooling3D(pool_size=(1,2,2), padding='same',name='x_a_Con')(x_a)
    #36x4
    x_a = layers.UpSampling3D(size=(1,2,2))(x_a_Con)
    x_a = layers.Conv3D(filters=2, kernel_size=(5,16,16), strides=(1,1, 1), padding='same',kernel_initializer=init, kernel_constraint=min_max_norm(min_value=-1.0, max_value=1.0))(x_a) #, kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
    x_a = layers.LeakyReLU()(x_a)
    #72x8
    x_a = layers.UpSampling3D(size=(2,2,2))(x_a)
    x_a = layers.Conv3D(filters=4, kernel_size=(5,16,16), strides=(1,1, 1), padding='same',kernel_initializer=init, kernel_constraint=min_max_norm(min_value=-1.0, max_value=1.0))(x_a) #
    x_a = layers.LeakyReLU()(x_a)
    #144x16
    x_a = layers.UpSampling3D(size=(2,2,2))(x_a)
    x_a = layers.Conv3D(filters=6, kernel_size=(5,16,16), strides=(1,1, 1), padding='same',kernel_initializer=init, kernel_constraint=min_max_norm(min_value=-1.0, max_value=1.0))(x_a) #,kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
    x_a = layers.LeakyReLU()(x_a)
    x_a = layers.Conv3D(filters=1, kernel_size=(1,1,1), strides=(1,1, 1), padding='same',kernel_initializer=init, kernel_constraint=min_max_norm(min_value=-1.0, max_value=1.0))(x_a) #kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
...
class RotationThetaWeightLayer(Layer): # a scaled layer
    ...
    def build(self, input_shape):
        ...
        self.W1 = self.add_weight(name='kernel', 
                                      shape=(1,),
                                      initializer=keras.initializers.Constant(value=90),
                                      trainable=True,
                                      constraint=min_max_norm(min_value=0, max_value=180))
...
def D3GenerateModel(n_filter=16, number_of_class=1, input_shape=(16,144,144,1),activation_last='sigomid', metrics=['mse', 'binary_accuracy', dice_coef], loss=[CC_loss, 'binary_crossentropy'], dropout=0.05, init='glorot_uniform', two_output=False):
   ...
   #B
    ...
    #x000 = Layers.AttentionLayer(class_nbr=number_of_class)(x)
    #x000 = Lambda(lambda x:(x-K.min(x))/(K.max(x)-K.min(x)))(x000)
    ...
    for comb in combination_list:
        ...
        x00 = layers.Lambda(lambda x: squash(x))(x00)
    ...
    x = Lambda(lambda x:(x-K.min(x))/(K.max(x)-K.min(x)))(x)
    x = layers.Lambda(lambda x: x/K.sum(x)) (x)
    x = layers.Reshape(target_shape=[reduce_filter,-1])(x)
    x = layers.Conv1D(filters=1, kernel_size=reduce_filter, strides=reduce_filter, kernel_initializer=init)(x)
    ...
    model.compile(optimizer=optimizer,loss=loss, metrics={'prediction' :metrics, 'autoencoder': 'mse'}, loss_weights=[0.03,1.0])#categorical_crossentropy loss_weights=[0.3,1.]
    return model
...
def D3GenerateModel_09_03_2019(n_filter=16, number_of_class=1, input_shape=(16,144,144,1),activation_last='softmax', metrics=['mse', 'binary_accuracy', dice_coef], loss=[CC_loss, 'binary_crossentropy'], dropout=0.05, init='glorot_uniform', two_output=False):
    ...
    mixture_dist = layers.Lambda(lambda x: ((x-K.min(x))/(K.max(x)-K.min(x))), name='RotationInvariant_DIST_min_max')(mixture_dist)
    '''
    x = layers.concatenate([dist_img, stroma,mixture])
    #16x144x144x1--> 8x72x72x16
    filter_size = n_filter
    ...
    import yogi #yogi.Yogi(lr=lr) # 
    optimizer = yogi.Yogi(lr=lr) #optimizers.adam(lr=lr, amsgrad=True)#yogi.Yogi(lr=lr) 
    model.compile(optimizer=optimizer,loss=loss, metrics={'prediction' :metrics, 'autoencoder': 'mse'}, loss_weights=[0.05,1.])#categorical_crossentropy loss_weights=[0.3,1.]loss_weights=[0.05,1.]
    return model

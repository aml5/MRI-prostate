...
def AutoEncoder(x,init):
    x_a = layers.Conv3D(filters=8, kernel_size=(5,5,5), strides = (1,1,1), kernel_initializer=init, padding='same', kernel_constraint=min_max_norm(min_value=0.0, max_value=1))(x) #, kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    x_a = layers.MaxPooling3D(pool_size=(2,2,2), padding='same')(x_a)
    #36x4
    x_a = layers.Conv3D(filters=4, kernel_size=(5,5,5), strides=(1,1, 1), padding='same',kernel_initializer=init, kernel_constraint=min_max_norm(min_value=0.0, max_value=1))(x_a) #, kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    x_a = layers.MaxPooling3D(pool_size=(2,2,2), padding='same')(x_a)
    #18x4
    x_a = layers.Conv3D(filters=1, kernel_size=(5,5,5), strides=(1,1, 1), padding='same',kernel_initializer=init, kernel_constraint=min_max_norm(min_value=0.0, max_value=1))(x_a) #, kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    ...
    #144x16
    x_a = layers.UpSampling3D(size=(2,2,2))(x_a)
    x_a = layers.Conv3D(filters=8, kernel_size=(5,5,5), strides=(1,1, 1), padding='same',kernel_initializer=init, kernel_constraint=min_max_norm(min_value=0.0, max_value=1))(x_a) #,kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    x_a = layers.Conv3D(filters=1, kernel_size=(5,5,5), strides=(1,1, 1), padding='same',kernel_initializer=init, kernel_constraint=min_max_norm(min_value=0.0, max_value=1))(x_a) #kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
...
def D3GenerateModel(n_filter=16, number_of_class=1, input_shape=(16,144,144,1),activation_last='softmax', metrics=['mse', 'binary_accuracy', dice_coef], loss=[CC_loss, 'binary_crossentropy'], dropout=0.05, init='glorot_uniform', two_output=False):
    ...
    dist_img = layers.Lambda(lambda x: (K.abs(x[0]-x[1]) / (K.abs(x[0]*x[2])+1)) * (1-x[3]), name='input_centered_and_divided')([input_x,loss_opt,y_auto,mask])
    ...
    stroma = layers.Lambda(lambda x: ((x[0]*x[1])**(x[2])), name='stroma')([input_x_t,y_auto,mask])

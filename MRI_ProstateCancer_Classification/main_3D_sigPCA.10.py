...
def D3GenerateModel(n_filter=16, number_of_class=1, input_shape=(16,144,144,1),activation_last='sigomid', metrics=['mse', 'binary_accuracy', dice_coef], loss=['mse', 'binary_crossentropy'], dropout=0.05, init='glorot_uniform', two_output=False):
    ...
    x = layers.Activation('relu')(x)#cyclical_learning_rate.SineReLU()(x)
    ...
    optimizer = yogi.Yogi(lr=lr)# optimizers.RMSprop(lr=lr)#yogi.Yogi(lr=lr) 
    model.compile(optimizer=optimizer,loss=loss, metrics={'prediction' :metrics, 'autoencoder': 'mse'}, loss_weights=[0.3,1.0])#categorical_crossentropy loss_weights=[0.3,1.]
    return model
...
def D3GenerateModel_09_03_2019(n_filter=16, number_of_class=1, input_shape=(16,144,144,1),activation_last='softmax', metrics=['mse', 'binary_accuracy', dice_coef], loss=[CC_loss, 'binary_crossentropy'], dropout=0.05, init='glorot_uniform', two_output=False):
    ...
    model.compile(optimizer=optimizer,loss=loss, metrics={'prediction' :metrics, 'autoencoder': 'mse'}, loss_weights=[0.05,1.])#categorical_crossentropy loss_weights=[0.3,1.]
    return model

...
np.random.seed(seed)
#General Model
def D3GenerateModel(n_filter=16, number_of_class=1, input_shape=(16,144,144,1),activation_last='sigmoid', metrics=['mse', 'acc', dice_coef, recall_at_thresholds, precision_at_thresholds], loss='binary_crossentropy', dropout=0.05, init='glorot_uniform', two_output=False):
    ...
    #1 level
    x = layers.Conv3D(filters=filter_size, kernel_size=(3,3,3), strides = (1,1,1), kernel_initializer=init, padding='same')(input_x)
    ...
    for index ,kernel_sizes in enumerate([
        ...
        for kernel_size in (kernel_sizes):
            ...
            x = layers.SpatialDropout3D(0.01)(x)
            counter = counter+1
        conv_list.append(x)
    ...
    x = layers.Conv1D(filters=number_of_class, kernel_size=filter_size*8, strides=filter_size*8, kernel_initializer=init, activation=activation_last)(x)#, kernel_regularizer=l2(0.001))(x)
    y = layers.Flatten()(x)
    #Classification
...
def Run(categorical=False, Normalize=False, shuffle=True):
    ...
    for lr in [1e-4, 1e-5, 1e-6, 1e-7]:
        for class_weight_c in [1,2,3,5]:
            print('Hyperparameter:', 'lr:',  lr, 'class_weight', class_weight_c)
            par_batch_size=16
            
            model = D3GenerateModel(number_of_class=1, input_shape=(16,144,144,1), activation_last='sigmoid')#KerasClassifier(build_fn=GenerateModel, epochs=20, batch_size=32, verbose=1)
            if counter==0:
                model.summary()
                model.count_params()
                counter+=1

            iterations = (X_train.shape[0]*2)
            valid_iteration = (X_valid.shape[0]*1)
            '''   
            ...
            '''
            log = callbacks.CSVLogger('./result_binary/%s_%s_log.csv' % (lr, class_weight_c))
            tb = callbacks.TensorBoard(log_dir='./result_binary/%s_%s_tensorboard-logs' % (lr,class_weight_c),
                                            batch_size=par_batch_size, histogram_freq=False)
            clr = cyclical_learning_rate.CyclicLR(base_lr=lr, max_lr=6e-3,
                                        step_size=2*iterations)#, mode='exp_range')
            
            checkpoint = callbacks.ModelCheckpoint('./result_binary/%s_%s_weights-{epoch:02d}.h5' %(lr, class_weight_c), monitor='val_dice_coef', #monitor='val_ucent_jaccard_distance',
                                                        save_best_only=False, save_weights_only=True, mode='max',verbose=1)
            model.fit_generator(GenerateEqualPositiveAndNegativeValue(X_train,Y_train, batch_size=par_batch_size, max_iteration_per_epoch=iterations),
                                steps_per_epoch=(iterations //par_batch_size), #X_train.shape[0]
                                epochs=10,
                                class_weight={0:1, 1:class_weight_c},
                                validation_data=GenerateEqualPositiveAndNegativeValue(X_valid,Y_valid, batch_size=16, max_iteration_per_epoch=valid_iteration),
                                validation_steps=X_valid.shape[0]//par_batch_size,
                                callbacks=[log, tb, checkpoint,clr ])#,lr_decay, reduce_lr])
            del model
            K.clear_session()
Run()

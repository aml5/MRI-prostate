...
def contractive_loss(y_pred, y_true):
    ...
    W = K.variable(value=model.get_layer('x_a_Con').get_weights()[0])  # N x N_hidden
    W = K.transpose(W)  # N_hidden x N
    h = model.get_layer('x_a_Con').output
...
def AutoEncoder(x,init):
    x_a = layers.Conv3D(filters=6, kernel_size=(5,5,5), strides = (1,1,1), kernel_initializer=init, padding='same', kernel_constraint=min_max_norm(min_value=0.00001, max_value=4))(x) #, kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    x_a = layers.MaxPooling3D(pool_size=(2,2,2), padding='same')(x_a)
    #36x4
    x_a = layers.Conv3D(filters=3, kernel_size=(5,5,5), strides=(1,1, 1), padding='same',kernel_initializer=init, kernel_constraint=min_max_norm(min_value=0.00001, max_value=4))(x_a) #, kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    x_a = layers.MaxPooling3D(pool_size=(2,2,2), padding='same')(x_a)
    #18x4
    x_a = layers.Conv3D(filters=1, kernel_size=(5,5,5), strides=(1,1, 1), padding='same',kernel_initializer=init, kernel_constraint=min_max_norm(min_value=0.00001, max_value=4))(x_a) #, kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    x_a_Con = layers.MaxPooling3D(pool_size=(1,2,2), padding='same',name='x_a_Con')(x_a)
    #36x4
    x_a = layers.UpSampling3D(size=(1,2,2))(x_a_Con)
    x_a = layers.Conv3D(filters=2, kernel_size=(5,5,5), strides=(1,1, 1), padding='same',kernel_initializer=init, kernel_constraint=min_max_norm(min_value=0.00001, max_value=4))(x_a) #, kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    #72x8
    x_a = layers.UpSampling3D(size=(2,2,2))(x_a)
    x_a = layers.Conv3D(filters=4, kernel_size=(5,5,5), strides=(1,1, 1), padding='same',kernel_initializer=init, kernel_constraint=min_max_norm(min_value=0.00001, max_value=4))(x_a) #
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    #144x16
    x_a = layers.UpSampling3D(size=(2,2,2))(x_a)
    x_a = layers.Conv3D(filters=6, kernel_size=(5,5,5), strides=(1,1, 1), padding='same',kernel_initializer=init, kernel_constraint=min_max_norm(min_value=0.00001, max_value=4))(x_a) #,kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    x_a = layers.Conv3D(filters=1, kernel_size=(5,5,5), strides=(1,1, 1), padding='same',kernel_initializer=init, kernel_constraint=min_max_norm(min_value=0.00001, max_value=4))(x_a) #kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
...
def Custom_loss(y_true, y_pred, smooth=1.):
    y_true_f = K.flatten(y_true)
    y_pred_f = K.flatten(y_pred)
    intersection = K.sqrt(K.square(K.sum(y_true_f - y_pred_f)*4))
    dce = intersection
    loss = dce**2+dce**3-dce**4
    return loss
...
class RotationThetaWeightLayer(Layer): # a scaled layer
    ...
    def build(self, input_shape):
        ...
        self.W1 = self.add_weight(name='kernel', 
                                      ...
                                      initializer='uniform',
                                      trainable=True,
                                      constraint=min_max_norm(min_value=1, max_value=360))
    ...
    def call(self, x):
        ...
        a, b = x
        #3.14159265359/self.W1, 3.14159265359/self.W2
        return K.cos(self.W1) * (-2) * K.exp(-(a**2+b**2)) + K.sin(self.W2) * (-2) * b * K.exp(-(a**2+b**2))

def D3GenerateModel(n_filter=16, number_of_class=1, input_shape=(16,144,144,1),activation_last='sigomid', metrics=['mse', 'binary_accuracy', dice_coef], loss=[CC_loss, 'binary_crossentropy'], dropout=0.05, init='glorot_uniform', two_output=False):
    ...
    mask = Lambda(lambda x:K.greater_equal(x, K.mean(x)), name='MASK')(y_auto)
    ...
    #Mask
    input_x_centered = layers.Lambda(lambda x: ((x[0]-K.mean(x[1]))/K.std(x[1]))*x[2], name='input_centered_by_autoencoder')([input_x_t,loss_opt_t,mask])
    #INVERSE
    ...
    #stroma = layers.Lambda(lambda x: ((x-K.min(x))/(K.max(x)-K.min(x))), name='Stroam_min_max')(stroma)
    ...
    x = layers.Conv3D(filters=filter_size, kernel_size=(3,5,5), strides = (1,1,1), kernel_initializer=init, padding='same')(x)
    x = cyclical_learning_rate.SineReLU()(x)
    x = layers.Conv3D(filters=filter_size, kernel_size=(3,5,5), strides=(1,1, 1), 
    ...
    for index ,kernel_sizes in enumerate([
        ...
        for kernel_size in (kernel_sizes):
            x = layers.Conv3D(filters=(filter_size*2), kernel_size=kernel_size, 
    ...
    for comb in combination_list:
        ...
        x00 = Lambda(lambda x: squash(x))(x00)
    ...
    model.compile(optimizer=optimizer,loss=loss, metrics={'prediction' :metrics, 'autoencoder': 'mae'}, loss_weights=[0.05,1.])#categorical_crossentropy loss_weights=[0.3,1.]
    return model
'''
alpha = K.variable(0.1)
beta = K.variable(0.5)
from keras.callbacks import Callback
class ModifyLossWeight(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        self.alpha = K.clip(self.alpha, 0.1, 1)
        self.beta = K.clip(self.beta, 0.1, 2)
'''
def D3GenerateModel_09_03_2019(n_filter=16, number_of_class=1, input_shape=(16,144,144,1),activation_last='softmax', metrics=['mse', 'binary_accuracy', dice_coef], loss=[CC_loss, 'binary_crossentropy'], dropout=0.05, init='glorot_uniform', two_output=False):
    #Extract unique structures
    input_x = layers.Input(shape=input_shape,name='Input_layer', dtype = 'float32')
    loss_opt,core_x = AutoEncoder(input_x, init)
    #Normalize input
    input_x_t = layers.Lambda(lambda x: ((x+1)/2))(input_x)
    #Normalize Encoder data, decoder data 
    loss_opt_t = layers.Lambda(lambda x: ((x+1)/2), name='loss_opt_t')(loss_opt)
    
    #Generate mask.
    y_auto = layers.UpSampling3D(size=(4,8,8))(core_x)
    mask = Lambda(lambda x:K.less_equal(x, K.mean(x)*0.9))(y_auto)
    mask = Lambda(lambda x:K.cast(x, 'float32'))(mask)
    ...
    for comb in combination_list:
        ...
        kernel_size_2 = (1, reduce(lambda x, y: x*y, comb_2))
        print(kernel_size_1,kernel_size_2)
        x00 = layers.Conv2D(filters=reduce_filter, kernel_size=kernel_size_1, kernel_initializer=init, strides=kernel_size_1, kernel_constraint=min_max_norm(min_value=-1, max_value=1))(x) #1296
        x00 = layers.BatchNormalization()(x00)
        x00 = layers.Activation('relu')(x00)
        x00 = layers.Conv2D(filters=reduce_filter, kernel_size=kernel_size_2, kernel_initializer=init, strides=kernel_size_2, kernel_constraint=min_max_norm(min_value=-1, max_value=1))(x00) #1296
        x00 = layers.BatchNormalization()(x00)
        x00 = layers.Lambda(lambda x: softmax(x))(x00)
        list_of_conv2D.append(x00)
    ...
    import yogi #yogi.Yogi(lr=lr) # 
    optimizer = yogi.Yogi(lr=lr) #optimizers.adam(lr=lr, amsgrad=True)#yogi.Yogi(lr=lr) 
    model.compile(optimizer=optimizer,loss=loss, metrics={'prediction' :metrics, 'autoencoder': 'mse'}, loss_weights=[0.03,1.])#categorical_crossentropy loss_weights=[0.3,1.]
    return model
...
def D3GenerateModel_v2(n_filter=16, number_of_class=1, input_shape=(16,144,144,1),activation_last='sigmoid', metrics=['mse', 'acc', dice_coef, recall_at_thresholds, precision_at_thresholds], loss=['mse','binary_crossentropy'], dropout=0.05, init='glorot_uniform', two_output=False):
    ...
    #TRY loss_weight=[0.03,1.0]
    model.compile(optimizer=optimizer,loss=loss, metrics={'prediction' :metrics, 'autoencoder': 'mse'}, loss_weights=[0.02,1.])#categorical_crossentropy
    return model
...
def Run(categorical=False, Normalize=False, shuffle=True):
    ...
    log = callbacks.CSVLogger('./result_recent_02_10/log.csv')
    ...
    model.fit_generator(GenerateEqualPositiveAndNegativeValue(X_train,Y_train, batch_size=16, max_iteration_per_epoch=iterations, RunApplyAugmentation=False),
                        ...
                        callbacks=[log, tb, checkpoint,clr]) #,ModifyLossWeight(alpha,beta)])

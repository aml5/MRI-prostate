...
def AutoEncoder(x,init):
    x_a = layers.Conv3D(filters=6, kernel_size=(5,5,5), strides = (1,1,1), kernel_initializer=init, padding='same')(x) #, kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    x_a = layers.MaxPooling3D(pool_size=(2,2,2), padding='same')(x_a)
    #36x4
    x_a = layers.Conv3D(filters=3, kernel_size=(5,5,5), strides=(1,1, 1), padding='same',kernel_initializer=init)(x_a) #, kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    x_a = layers.MaxPooling3D(pool_size=(2,2,2), padding='same')(x_a)
    #18x4
    x_a = layers.Conv3D(filters=1, kernel_size=(5,5,5), strides=(1,1, 1), padding='same',kernel_initializer=init)(x_a) #, kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    x_a_Con = layers.MaxPooling3D(pool_size=(1,2,2), padding='same',name='x_a_Con')(x_a)
    #36x4
    x_a = layers.UpSampling3D(size=(1,2,2))(x_a_Con)
    x_a = layers.Conv3D(filters=2, kernel_size=(5,5,5), strides=(1,1, 1), padding='same',kernel_initializer=init)(x_a) #, kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    #72x8
    x_a = layers.UpSampling3D(size=(2,2,2))(x_a)
    x_a = layers.Conv3D(filters=4, kernel_size=(5,5,5), strides=(1,1, 1), padding='same',kernel_initializer=init)(x_a) #
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    #144x16
    x_a = layers.UpSampling3D(size=(2,2,2))(x_a)
    x_a = layers.Conv3D(filters=6, kernel_size=(5,5,5), strides=(1,1, 1), padding='same',kernel_initializer=init)(x_a) #,kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
    x_a = cyclical_learning_rate.SineReLU()(x_a)
    x_a = layers.Conv3D(filters=1, kernel_size=(5,5,5), strides=(1,1, 1), padding='same',kernel_initializer=init)(x_a) #kernel_constraint=min_max_norm(min_value=0.0, max_value=1)
...
def D3GenerateModel(n_filter=16, number_of_class=1, input_shape=(16,144,144,1),activation_last='sigomid', metrics=['mse', 'binary_accuracy', dice_coef], loss=[CC_loss, 'binary_crossentropy'], dropout=0.05, init='glorot_uniform', two_output=False):
    ...
    model.compile(optimizer=optimizer,loss=loss, metrics={'prediction' :metrics, 'autoencoder': 'mse'}, loss_weights=[0.1,1.])#categorical_crossentropy loss_weights=[0.3,1.]
    return model

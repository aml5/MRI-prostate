...
np.random.seed(seed)
#General Model
def D3GenerateModel(n_filter=16, number_of_class=1, input_shape=(16,144,144,1),activation_last='softmax', metrics=['mse', 'acc', dice_coef, recall_at_thresholds, precision_at_thresholds], loss='kullback_leibler_divergence', dropout=0.05, init='glorot_uniform', two_output=False):
    ...
    x = layers.Conv3D(filters=filter_size, kernel_size=(3,3,3), strides=(1,1, 1), 
    ...
    x = layers.Conv3D(filters=filter_size*4, kernel_size=(3,3,3), strides=(2,2, 2), kernel_initializer=init,
    ...
    x = layers.Conv1D(filters=2, kernel_size=filter_size*4, strides=filter_size*4, kernel_initializer=init, kernel_regularizer=l2(0.001))(x)
    x = layers.Softmax()(x)
    y = layers.Flatten()(x)
    #Classification    

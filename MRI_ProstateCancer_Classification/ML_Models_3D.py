...
def ApplyAugmentation(d3_img, type_of_augmentation=None, dict_parameter=None, seed=1):
    ...
    if dict_parameter is None:
        dict_parameter={'rotation_xy':[-8,8],
                        'rotation_zx' :[-8,8],
                        'rotation_zy' :[-8,8],
                        'zooming':[1.05,1.15]
                        #'down_scale':[1,0.8]
    ...
    if bool_val=='T':
        transformation = augment.create_identity_transformation(new_3d_img.shape)
        # jitter in 3D
        ...
        new_3d_img = augment.apply_transformation(new_3d_img, transformation)
    '''
...
def contrastive_loss(y_true, y_pred):
    ...
    return K.mean(y_true * sqaure_pred + (1 - y_truesoftmax) * margin_square)
...
np.random.seed(seed)
#General Functions
def D3GenerateModel(n_filter=16, number_of_class=1, input_shape=(16,144,144,1),activation_last='sigmoid', metrics=['mse', 'acc', dice_coef, recall_at_thresholds, precision_at_thresholds, auc_roc], loss='binary_crossentropy', dropout=0.05, init='glorot_uniform', two_output=False):
    filter_size =n_filter
    input_x = layers.Input(shape=input_shape,name='Input_layer', dtype = 'float32')
    #1 level
    x = layers.Conv3D(filters=filter_size*2, kernel_size=(3,3,3), strides = (1,1,1), padding='same')(input_x)
    x = layers.LeakyReLU()(x)
    x = layers.Conv3D(filters=filter_size*4, kernel_size=(3,3,3), strides=(1,1, 1), 
                                        padding='same')(x)
    x = layers.LeakyReLU()(x)
    ...
    for index ,kernel_sizes in enumerate([
        ...
        for kernel_size in (kernel_sizes):
            x = layers.Conv3D(filters=(filter_size*8), kernel_size=kernel_size, strides =(1,1,1), padding='same', name='Conv3D_%s' % (counter))(x)
    ...
    x = layers.Conv1D(filters=2, kernel_size=128, strides=128, activation='sigmoid')(x)
    y = layers.Flatten()(x)
    #Classification    
...
def GenerateEqualPositiveAndNegativeValue(X,Y, batch_size=16, RunNormalize=True):
...
def Run(categorical=True, Normalize=False, shuffle=True):
    ...
    #Call model
    model = D3GenerateModel(number_of_class=2)#KerasClassifier(build_fn=GenerateModel, epochs=20, batch_size=32, verbose=1)
    model.summary()
    model.count_params()
    ...
    checkpoint = callbacks.ModelCheckpoint('./weights-{epoch:02d}.h5', monitor='val_dice_coef', #monitor='val_ucent_jaccard_distance',
                                                save_best_only=False, save_weights_only=True, mode='max',verbose=1)
    model.fit_generator(GenerateEqualPositiveAndNegativeValue(X_train,Y_train, 16),
                        steps_per_epoch=(10000 // 16), #X_train.shape[0]
                        epochs=50,
                        class_weight={0:1, 1:2},
                        validation_data=D3ImageGenerator(X_valid,Y_valid, 16, RunNormalize=True),

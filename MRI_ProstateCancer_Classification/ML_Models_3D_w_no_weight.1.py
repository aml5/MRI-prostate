...
np.random.seed(seed)
#General Model
def D3GenerateModel(n_filter=16, number_of_class=1, input_shape=(16,144,144,1),activation_last='softmax', metrics=['mse', 'acc', dice_coef, recall_at_thresholds, precision_at_thresholds], loss='binary_crossentropy', dropout=0.05, init='glorot_uniform', two_output=False):
    ...
    x = layers.Conv1D(filters=2, kernel_size=filter_size*8, strides=filter_size*8, kernel_initializer=init, activation='softmax')(x)#, kernel_regularizer=l2(0.001))(x)
    y = layers.Flatten()(x)
    #Classification    
...
def Run(categorical=True, Normalize=False, shuffle=True):
    ...
    model.summary()
    model.count_params()
    iterations = (X_train.shape[0]*4)
    valid_iteration = (X_valid.shape[0]*1)
    log = callbacks.CSVLogger('./result/g_adam_decay_reduce_clog.csv')
    tb = callbacks.TensorBoard(log_dir='./result/adam_decay_tensorboard-logs',
    ...
    '''
    clr = cyclical_learning_rate.CyclicLR(base_lr=1e-6, max_lr=6e-6,
                                step_size=2*iterations, mode='exp_range')
    
    checkpoint = callbacks.ModelCheckpoint('./result/adam_decay_reduce_weights-{epoch:02d}.h5', monitor='val_dice_coef', #monitor='val_ucent_jaccard_distance',

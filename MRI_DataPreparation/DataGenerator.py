...
def Combine_two_csv_by_lesion_finding_id(csv_file_images, csv_finding_classification, csv_ktrans=None, filename=None):
    ...
    finding_classification['ProxID_fid'] = finding_classification['ProxID'] + '_' + (finding_classification['fid']).astype(str)
...
#
#######
def PrepareData(to_store=None, folder_with_images=None,folder_with_ktrans=None, csv_for_finding_classification=None, csv_for_finding_images=None, csv_ktrans=None):
    ...
    #Merge two csv files
    Combine_two_csv_by_lesion_finding_id(csv_for_finding_images, csv_for_finding_classification,csv_ktrans=csv_ktrans, filename=f.name)
...
def KtranCoRegistration(ktrans, mri, verbose=False):
    ...
    transform = sitk.CenteredTransformInitializer(fixed_image, 
                                                moving_image, 
                                                sitk.AffineTransform(fixed_image.GetDimension()),# Euler3DTransform(), 
        ...
        if found==True:
            ...
            if considered_mri_p=='ktrans':
                ...
                if verbose:
                    ...
                    plt.show()
                ktrans_mri =  GenerateSitkImage(pts_list[index]['ktrans'][data_storage_identifier],ktrans_mode=True, Normalize=False, verbose=verbose)
...
def GenerateImageWithMultiChannels(img,
    ...
    for index, channel in enumerate(channels):
        ...
        if channel not in img:
            ...
            window_size = int(round(Window_size_mm[0]/float(VoxelSpacing[0]))),int(round(Window_size_mm[1]/float(VoxelSpacing[1])))
            ...
            if verbose:
                ...
                rect = patches.Rectangle((start_x,start_y),window_size[0],window_size[1],linewidth=1,edgecolor='r',facecolor='none')
